# Feature: core-algorithms-and-data-structures-series
...
- **Client / Blog Data Layer:**
  ...
  - **Standardized Evaluation Framework (Article #01):**
    ...
    - Serves as the quantitative benchmark for all subsequent algorithm and data structure articles in the series.
...
### Part C: Data Structures (`post-48` - `post-54`)

1. **`post-48` (`data-structure-array-dynamic-array-vector-cpp`):** Array & Dynamic Array / Vector (Contiguous memory, direct pointer indexing $O(1)$, geometric capacity doubling $O(1)$ amortized, and custom C++ Vector).
2. **`post-49` (`data-structure-linked-list-singly-doubly-sentinel-cpp`):** Linked List (Singly vs Doubly, Sentinel Dummy Head/Tail pattern eliminating boundary checks, $O(1)$ splice, and C++ pointer management).
3. **`post-50` (`data-structure-stack-lifo-callstack-cpp`):** Stack (LIFO principle, runtime Call Stack model, Dynamic Array vs Linked List implementations, and Valid Parentheses parser).
4. **`post-51` (`data-structure-queue-fifo-circular-ring-buffer-cpp`):** Queue & Circular Queue (FIFO principle, Ring Buffer modulo arithmetic eliminating false overflows, Deque extensions, and C++ implementation).
5. **`post-52` (`data-structure-hash-table-collision-chaining-open-addressing-cpp`):** Hash Table (Uniform hash distribution, Separate Chaining vs Open Addressing, Load Factor & dynamic Rehashing $O(1)$).
6. **`post-53` (`data-structure-binary-search-tree-traversals-cpp`):** Tree & Binary Search Tree (BST Invariants, 4 Traversal algorithms Pre/In/Post/Level-order BFS, $O(\log N)$ Search/Insert/Delete routines).
7. **`post-54` (`data-structure-graph-representations-bfs-dfs-cpp`):** Graph (Vertices & Edges, Adjacency Matrix $O(V^2)$ vs Adjacency List $O(V + E)$, Breadth-First & Depth-First search algorithms).
...
- **Strict Schema Compliance:**
  ...
  - Integrated valid, vector-rendered Mermaid.js diagrams illustrating state transitions, memory layouts, tree partitioning, and execution lifecycles.
...
- `src/data/blog/vi/2026/09.json`: Complete 28 Computer Science foundational articles (8 Basic Algo + 13 Advanced Algo + 7 Data Structure) in Vietnamese with Mermaid diagrams and C++ implementations.
- `src/data/blog/en/2026/09.json`: Complete 28 Computer Science foundational articles (8 Basic Algo + 13 Advanced Algo + 7 Data Structure) in English with Mermaid diagrams and C++ implementations.
...
- `docs/features/core-algorithms-series.md`: Technical documentation for Algorithms & Data Structures series.
